...
public class FocusEffect extends ShadowEffect {
    ...
    public BufferedImage applyEffect(BufferedImage src, BufferedImage dst, int w, int h) {
        ...
        byte[] tmpBuf3 = getArrayCache().getTmpByteArray3(tmpW * tmpH);
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int x = 0; x < w; x++) {
                int dx = x + tmpOffX;
                tmpBuf1[offset + dx] = tmpBuf3[offset + dx] = (byte) ((lineBuf[x] & 0xFF000000) >>> 24);
            }
        }
        // blur
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int x = 0; x < w; x++) {
                int srcX = x + tmpOffX;
                int i = shadowOffset + (srcX - offsetX);
                // Multiply the glow alpha by the inverse of the source image alpha.
                int srcAlpha = ((int) tmpBuf3[i]) & 0xFF;
                int intAlpha = ((int) tmpBuf1[i]) & 0xFF;
                byte alpha = (byte) ((intAlpha * (255 - srcAlpha)) / 255);
                lineBuf[x] = alpha << 24 | red << 16 | green << 8 | blue;
            }
